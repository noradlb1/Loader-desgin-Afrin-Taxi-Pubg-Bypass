...
//
//   // static ImVec4 rd = ImColor(1.000f, 0.0f, 0.000f, 1.000f); ImVec4(r / 255.0, g / 255.0, b / 255.0, a / 255.0);
//    static ImVec4 rd = ImVec4(64 / 255.0, 83 / 255.0, 39 / 255.0, 255 / 255.0);
//    static ImVec4 blr = { 0.000f, 0.984f, 0.984f, 1.000f };
//    static ImVec4 gr = { 0.158f, 0.992f, 0.000f, 1.000f };
//    const ImU32 col = GetColorU32((hovered) ? gr : rd);
//
//    if (*v)
//    {
//
...
//
//        {
//            window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 0, check_bb.Min.y + 1), ImVec2(check_bb.Min.x + 17, check_bb.Min.y + 17), col, 4);
//
//        }
//        else
//        {
//
//            // ImGui::PushStyleColor(col, (hovered || held) ? blr : rd);
//            window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 0, check_bb.Min.y + 1), ImVec2(check_bb.Min.x + 17, check_bb.Min.y + 17), ImColor(1.000f, 0.0f, 0.000f, 1.000f), 4);
...
//        {
//
//            window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 0, check_bb.Min.y + 1), ImVec2(check_bb.Min.x + 17, check_bb.Min.y + 17), ImColor(40, 40, 40, 255), 4);
//        }
//    }
//    //ImVec4 hov = { 1.000f, 1.000f, 1.000f, 1.000f };
//    //static ImVec4 inactive = ImVec4(150 / 255.f, 150 / 255.f, 150 / 255.f, 255 / 255.f);
//    ImVec4 hov =  ImVec4(0.80f, 0.80f, 0.83f, 1.00f);
//    static ImVec4 inactive = ImVec4(0.80f, 0.80f, 0.83f, 1.00f);
//
//    if (label_size.x > 0.0f)
//        ImGui::PushStyleColor(ImGuiCol_Text, *v ? hov : inactive);
//    RenderText(ImVec2(check_bb.Max.x - 10, check_bb.Min.y + 1.5f), label);
//    ImGui::PopStyleColor(1);
//    return pressed;
//}
//
...
//    const ImVec2 label_size = CalcTextSize(label, NULL, true);
//    const ImVec2 pading = ImVec2(2, 2);
//    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(160, label_size.y + style.FramePadding.y / 2));
//    ItemSize(check_bb, style.FramePadding.y);
//    ImRect total_bb = check_bb;
//    if (label_size.x > 0)
//        SameLine(0, style.ItemInnerSpacing.x);
...
//    const float pad = ImMax(1.0f, (float)(int)(check_sz / 4.f));
//    //window->DrawList->AddRectFilled(check_bb.Min+ImVec2(pad,pad), check_bb.Max-ImVec2(pad,pad), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
// //  draw->AddLine(ImVec2(check_bb.Min.x - 15, check_bb.Min.y), ImVec2(check_bb.Min.x - 15, check_bb.Max.y), ImColor(35, 35, 35));
// //   draw->AddLine(ImVec2(check_bb.Max.x + 15, check_bb.Min.y), ImVec2(check_bb.Max.x + 15, check_bb.Max.y), ImColor(35, 35, 35));
//
//
//    if (*v)
//    {
...
//        //window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + (check_bb.Max.x - check_bb.Min.x) / 2, check_bb.Min.y), check_bb.Max, GetColorU32(ImVec4(0.34f, 1.0f, 0.54f, 1.0f)), 0);
//        draw->AddRectFilled(ImVec2(check_bb.Min.x + 0, check_bb.Min.y + 1), ImVec2(check_bb.Min.x + 17, check_bb.Min.y + 17), ImColor(33, 212, 24), 4);
//
//    }
//    else
//    {
//
//        draw->AddRectFilled(ImVec2(check_bb.Min.x + 0, check_bb.Min.y + 1), ImVec2(check_bb.Min.x + 17, check_bb.Min.y + 17), ImColor(117, 19, 19, 255), 4);
//    }
//
//    if (label_size.x > 0.0f)
//        // RenderText(ImVec2(check_bb.Max.x + 25, check_bb.Min.y + 0), label);
//        return pressed;
//}
...
//
//    const ImVec2 label_size = CalcTextSize(label, NULL, true);
//    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(260, label_size.y + style.FramePadding.y * 2.0f));
//    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
//
//    ItemSize(total_bb, style.FramePadding.y);
//    if (!ItemAdd(total_bb, id, &frame_bb))
...
//    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : g.HoveredId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
//    RenderNavHighlight(frame_bb, id);
//    window->DrawList->sliderline(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(frame_bb.Max.x, frame_bb.Min.y + 10), ImColor(238, 238, 238), 2.0f);
//    window->DrawList->sliderline(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 10), ImColor(255, 255, 0, 255), 2.f);
//    // window->DrawList->AddLine(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 10), ImVec2(grab_bb.Min.x, frame_bb.Min.y + 10), ImColor(120, 173, 181), 2.f);
//
//     // Render grab
//    if (grab_bb.Max.x > grab_bb.Min.x)
//        //   window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x, grab_bb.Min.y + 3.5f), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 3.5f), ImColor(0, 173, 181), 3);
//        RenderArrow(ImVec2(grab_bb.Max.x - arrow_size + style.FramePadding.y+3, grab_bb.Min.y + style.FramePadding.y+3), ImGuiDir_Up);
//    //  rendera
//   // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
//    char value_buf[64];
//    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
//   // RenderTextClipped(ImVec2(frame_bb.Max.x - 35, frame_bb.Min.y - 23), ImVec2(frame_bb.Max.x + 20, frame_bb.Min.y + 20), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
//    
//    if (label_size.x > 0.0f)
//     //   RenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 10), label);
//
//    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, window->DC.ItemFlags);
//    return value_changed;
//}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 1.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
    ...
    char buf_display[64] = "Change key";
    ...
    if (*k != 0 && g.ActiveId != id) {
        ...
        strcpy_s(buf_display, "Press Key");
    }
...
bool    ImGui::BeginTabBar(const char* str_id, ImGuiTabBarFlags flags)
{
    ...
    ImRect tab_bar_bb = ImRect(window->DC.CursorPos.x, window->DC.CursorPos.y, window->WorkRect.Max.x, window->DC.CursorPos.y + g.FontSize + g.Style.FramePadding.y * 2.2);
    tab_bar->ID = id;
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    // Draw separator
    const ImU32 col = GetColorU32(ImVec4(0.566f, 0.520f, 0.000f, 1.000f));
    const float y = tab_bar->BarRect.Max.y - 1.0f;
    {
        const float separator_min_x = tab_bar->BarRect.Min.x - IM_FLOOR(window->WindowPadding.x * 0.5f);
        const float separator_max_x = tab_bar->BarRect.Max.x + IM_FLOOR(window->WindowPadding.x * 0.5f);
        window->DrawList->AddLine(ImVec2(separator_min_x, y), ImVec2(separator_max_x, y), col, 1.3f);
    }
    return true;
}
