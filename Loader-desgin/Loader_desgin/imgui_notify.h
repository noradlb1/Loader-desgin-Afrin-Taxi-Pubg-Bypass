...
#include <vector>
#include <string>
#include "C:\Users\GAN\Desktop\Loader-desgin\Loader_desgin\imgui-docking\font_awesome_5.h"
#include "C:\Users\GAN\Desktop\Loader-desgin\Loader_desgin\imgui-docking\fa_solid_900.h"
...
public:
	...
	float			startTime = 0.0f; // وقت البدء
	int				duration = NOTIFY_DEFAULT_DISMISS; // المدة بالمللي ثانية
...
public:
	...
	ImGuiToast(ImGuiToastType type, int dismiss_time = NOTIFY_DEFAULT_DISMISS)
	{
		...
		this->startTime = ImGui::GetTime(); // تعيين وقت البدء
		this->duration = dismiss_time; // تعيين المدة
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			const auto opacity = current_toast->get_fade_percent(); // Get opacity based on the current phase
			...
			// Render the toast content
			{
				...
				{
					SetCursorPosY(GetCursorPosY() + 5.f); // Move the cursor down to add padding
				}
				...
				float elapsedTime = currentTime - current_toast->startTime;
				float fillRatio = elapsedTime / (current_toast->duration / 1000.0f);
				ImGui::PushStyleColor(ImGuiCol_PlotHistogram, ImVec4(0.0f, 1.0f, 0.0f, 1.0f)); // اللون الأخضر

				ProgressBar(fillRatio, ImVec2(0.0f, 2.0f));
				ImGui::PopStyleColor();
				PopTextWrapPos();
			}
