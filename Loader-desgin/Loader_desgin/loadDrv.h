...
class DRIVER {
	...
	bool WriteDataToFile(const UCHAR pBuffer[], const DWORD dwSize, const std::string& strFileName, const DWORD dwCreationDisposition = CREATE_NEW)
	{
	 ...
	 bool existsFuile(const std::string& name)
	{
		struct stat buffer;
...
public:
		...
		const size_t szRawDriver = 0)
	{
		...
		if (existsFuile(m_strDriverFile)) {
			
		}
		else {
			(WriteDataToFile(m_pRawDriver, m_szRawDriver, m_strDriverFile));
		//essageBoxA(0, "Driver Not Found",0 , 0 );
		}
	...
	bool Load()
	{
		...
		else if(!bStartService && GetLastError() == ERROR_PATH_NOT_FOUND) 
		{ 
			DeleteService(hSc);
			Load();
		}
